...
%        For COL input, if TYPE is numerical integer indexes the outputs
%        will be split arrray (like 'split' bellow) but restriced to columns
%        specified in TYPE
...
%   21-Apr-2018: extend 'type' with split column numbers
%   19/Jul-2018: Output specific columns on split-array 
...
for k=1:nout
        ...
        switch lower(funk)
            ...
            otherwise
                try % Try to evaluate as it is a function name
                    split = getsplit(v_isrow, v, lgt);
                    if k<=nout-2 && ischar(outtype{k+1}) && strncmpi(outtype{k+1},'UniformOutput',13)
                        unif = outtype{k+2};
                        outtype(k+1:k+2) = {[]};
                    else
                        unif = false;
    ...
    elseif isnumeric(funk)
        % column output
        cols = funk;
        out{k} = getsplit(v_isrow, v(:,cols), lgt);
    elseif isa(funk,'function_handle')
...
function s = getsplit(v_isrow, v, lgt)
persistent SPLIT
if ischar(v_isrow) && strcmpi(v_isrow,'clean')
    SPLIT = [];
elseif ~iscell(SPLIT)
    if ~v_isrow
