function ft_data_cat = mergeFTTrials_MS(datapath,seshnums,stagenames)
for sesh = seshnums
    trialdefcat = [];
    for stage = 1:numel(stagenames)
                try

        X = load([datapath '\SMR Data\TrialDefs\Pilot' stagenames{stage} '_MS_' num2str(sesh) '.mat']); % Can you make all of the trialdefs have a standard name or else it gets really tricky to load in!
